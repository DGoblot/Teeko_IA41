...
class Ia:
    ...
    def alphaBeta(self, depth,niveau):
        ...
        if tmpGameJ.pions_restant > 0:#Il y a encore des pions non mis sur le plateau
            ...
            for l in range(0, 5):
                for c in range(0, 5):
                    if tmpGameJ.plateau[l][c]==0: # Si c'est vide on insère un pion
                        ...
                        tmp = self.min(tmpGameJ,niveau,depth - 1, alpha, beta,l,c) # appelle de la fonction min
            ...
            self.GameJ.plateau[meilleur_l][ meilleur_c]=self.player # Mettre a jour le plateau par la meilleur position du pion
          #Modifier le nombre de pion et aussi quel joueur va jouer
            ...
            self.GameJ.movePions(meilleur_l1, meilleur_c1, meilleur_l2, meilleur_c2)# Mettre a jour le plateau final
    ...
    def min(self,tmpGameJ,niveau ,depth, alpha, beta,x,y):
        ...
        if tmpGameJ.pions_restant + depth - self.profondeur > 0: #Pour savoir s'il y a encore des pions non utilisé
            ...
            for l in range(5):
                for c in range(5):
                    if tmpGameJ.plateau[l][c] == 0: #Si position est vide l'opposant met son pion
                        ...
                        tmp = self.max(tmpGameJ,niveau,depth - 1, alpha, beta,l,c) #Appelle de la fonction max
            ...
            for l in range(5):
                for c in range(5):
                    if tmpGameJ.plateau[l][c] == -1*self.player: # Si il existe un pion de l'adversaire a cette position

                        adjacents = tmpGameJ.getAdjacent(l, c) # Recherche des position adjacentes du pion

                        for ad in adjacents:
                            if tmpGameJ.moviePionsRobot(l, c, ad[0], ad[1], -1*self.player): #Mettre à jour le plateau temporelle
                                print("test")
                                tmp = self.max(tmpGameJ,niveau,depth - 1, alpha, beta,ad[0],ad[1]) # Appelle de la fonction max
            ...
            for l in range(5):
                for c in range(5):
                    if tmpGameJ.plateau[l][c] == self.player: # Si il existe un pion à cette position
                        ...
                        for ad in adjacents:
                            if tmpGameJ.moviePionsRobot(l, c, ad[0], ad[1],  self.player): #Mettre le pion sur le plateau temporelle
                                tmp = self.min(tmpGameJ,niveau,depth - 1, alpha, beta,ad[0],ad[1]) #Appelle de la fonction min
    ...
    def eval(self,tmpGameJ,verif,type):
        ...
        if type: # Si le joueur qui active la fonction eval est l'opposant
            value = -1 * value
